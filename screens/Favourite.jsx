    ...
    <View className="relative">
      <SafeAreaView className="mt-4 mx-3 h-full">
        <View className="flex-row items-center w-full fixed justify-start mb-5">
          ...
          <Text className="font-black text-xl tracking-[6px]">Favorites</Text>
        </View>

        <FlatList
          data={favData}
          renderItem={({ item }) => (
            <View className="flex-1 flex-row justify-between items-center p-4 mb-3  rounded-md shadow-xl bg-white shadow-slate-300 ">
              ...
              <View className="flex-1 mx-3 ">
                <Text className="font-bold text-sm text-[#6CB2EB]">
                  {item.title}
                </Text>
                <Text className="font-bold text-xs text-gray-600">
                  {item.supplier}
                </Text>
                <Text className="font-black text-xs text-gray-500">
                  $ {item.price}
                </Text>
              </View>
